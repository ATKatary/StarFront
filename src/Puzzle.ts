...
/*** Enums ***/
export enum SpaceStatus {Empty, Dot, Star}
export enum ModifyValue {Increment, Decrement, Constant}

export class Puzzle {
    /*** Representation ***/
    public  readonly isSolved: boolean;
    private readonly rowToStars: Map<number, number> = new Map<number, number>();
    private readonly columnToStars: Map<number, number> = new Map<number, number>();
    private readonly regionToStars: Map<number, number> = new Map<number, number>();
    private readonly regionToCells: Map<number, Array<number>> = new Map<number, Array<number>>();
    ...
    public constructor(private readonly cellToRegion: Map<number, number>, private readonly cellToSpaceStatus: Map<number, SpaceStatus>, private readonly puzzleSize: number) {
        this.isSolved = true;
        for (let i = 0; i < 2; i++) {
            for (const [cell, status] of cellToSpaceStatus){
                ...
                if (region === undefined) throw new Error("region does not exist");
                ...
                if (numRowStars === undefined) numRowStars = 0;
                if (numColStars === undefined) numColStars = 0;
                if (numRegionStars === undefined) numRegionStars = 0;
                ...
                else {if (numRowStars < REQUIRED_STARS || numColStars < REQUIRED_STARS || numRegionStars < REQUIRED_STARS) {this.isSolved = false; break;}}
            }
        }
        ...
        for (const [cell, region] of this.cellToRegion){
            ...
            if (!cellsEnclosedByRegion) this.regionToCells.set(region, [cell]);
            else {
                const cellSpaceStatus = cellToSpaceStatus.get(cell);
                if (cellSpaceStatus === SpaceStatus.Star) cellsEnclosedByRegion.unshift(cell);
                else cellsEnclosedByRegion.push(cell);
            }
        }
        this.checkRep();
      }
    ...
    private checkRep(): void{
        for (const [region, regionStars] of this.regionToStars){
            ...
            if (rowStars === undefined || regionStars === undefined || colStars === undefined) throw new Error("Something here is wrong");
            if (rowStars > REQUIRED_STARS || rowStars < 0) throw Error("The RI has been violated: expected rowStars <= 2");
            if (colStars > REQUIRED_STARS || colStars < 0) throw Error("The RI has been violated: expected colStars <= 2");
            if (regionStars > REQUIRED_STARS || regionStars < 0) throw Error("The RI has been violated: expected regionStars <= 2");
        }
    }
     ...
     */
    private modifyStarsMaps(cell: number, increment: ModifyValue): void{
        let valueChange = 0;
        if (increment === ModifyValue.Increment) valueChange = 1;
        else if (increment === ModifyValue.Decrement) valueChange = -1;
        ...
        if (numRowStars  === undefined || numColStars === undefined || numRegionStars === undefined) throw new Error("something went wrong with star count");
     ...
     * @returns the region associated with the index
     */
    public getRegion(cell: number): number{
        const region = this.cellToRegion.get(cell);
     ...
     */
    public changeStatus(cell: number, newStatus: SpaceStatus): Puzzle{
        let modifyValue; let revertValue;
        const currStatus = this.cellToSpaceStatus.get(cell);
        switch (currStatus) {
            case SpaceStatus.Dot || SpaceStatus.Empty: {
                if (newStatus === SpaceStatus.Star) [modifyValue, revertValue] = [ModifyValue.Increment, ModifyValue.Decrement]; 
                break;
            }
            case SpaceStatus.Star: {
                if (newStatus === SpaceStatus.Dot || newStatus === SpaceStatus.Empty) [modifyValue, revertValue] = [ModifyValue.Decrement, ModifyValue.Increment];
                break;
            }
            default: break;
        }
    ...
    public toString(): string {
        ...
        for (let region = 0; region < this.puzzleSize; region++){
            ...
            if (!cellsEnclosedByRegion) continue;
     ...
     */
    private _coordinates(cell: number): [row: number, col: number] {
