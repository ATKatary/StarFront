...
enum PuzzleGrammar {Size, Puzzle, Cell, Text, Number, Whitespace, Region}
...
export function parsePuzzle(puzzleString: string): Puzzle {
    ...
    for (const line of puzzleString.split('\n')) {
        if (line && line[0] !== '#') cleanedPuzzleString = cleanedPuzzleString.concat(line, '\n');
    }
    const parseTree: ParseTree<PuzzleGrammar> = parser.parse(cleanedPuzzleString);
...
function makeAbstractSyntaxTree(parseTree: ParseTree<PuzzleGrammar>): Puzzle {
    ...
    case PuzzleGrammar.Puzzle: // puzzle ::= size regions+;
        {   
            ...
            if (!size) throw new Error("Corrupt puzzle file");
            const [puzzleW, puzzleH] = [size.children[0], size.children[1]];
            if (!puzzleW || !puzzleH) throw new Error("Corrupt puzzle file");
            
            const w = Number.parseInt(puzzleW.text);
            const h = Number.parseInt(puzzleH.text);

            const regions: Array<ParseTree<PuzzleGrammar>> = parseTree.children.slice(1);
            const cellToRegion: Map<number, number> = new Map();
            const cellToSpaceStatus: Map<number, SpaceStatus> = new Map();

            for (let i = 0; i < regions.length; i++) {
                const region = regions[i];
                if (!region) throw new Error("Corrupt puzzle file");
                const cells = region.children;

                for (const cell of cells) {
                    const [row, col] = cell.children;
                    if (!row || !col) throw new Error("Corrupt puzzle file");
                    const index = (Number.parseInt(row.text) - 1) * h + (Number.parseInt(col.text) - 1);

                    cellToRegion.set(index, i);
                    cellToSpaceStatus.set(index, SpaceStatus.Empty);
                }
            }
            return new Puzzle(cellToRegion, cellToSpaceStatus, h);
        }
