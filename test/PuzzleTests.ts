...
import { Puzzle, SpaceStatus } from "../src/Puzzle";
...
describe('puzzle', function() {
  ...
  //     valid for kd-6-31-6.starb
    ...
    it("covers new status of a grid cell = Dot[no change], covers valid getRegion index", async function () {
        ...
        const fileContents = await fetch(`http://localhost:${server.port}/get/kd-1-1-1.starb`);
        const puzzle: Puzzle = parsePuzzle(await (fileContents.text()));
        puzzle.changeStatus(0, SpaceStatus.Dot);
        assert(puzzle.getRegion(0) === SpaceStatus.Empty);
    ...
    it("covers new status of a grid cell = Empty[no change], covers invalid getRegion index", async function () {
        ...
        const fileContents = await fetch(`http://localhost:${server.port}/get/kd-1-1-1.starb`);
        const puzzle: Puzzle = parsePuzzle(await (fileContents.text()));
        puzzle.changeStatus(1, SpaceStatus.Empty);
        assert(puzzle.getRegion(1) === SpaceStatus.Empty);
    ...
    it("covers new status of a grid cell = Star[no change]", async function () {
        ...
        const fileContents = await fetch(`http://localhost:${server.port}/get/kd-1-1-1.starb`);
        const puzzle: Puzzle = parsePuzzle(await (fileContents.text()));
        puzzle.changeStatus(1, SpaceStatus.Star);
        assert(puzzle.getRegion(1) === SpaceStatus.Empty);
    ...
    it("covers toString output valid for kd-1-1-1.starb", async function () {
        ...
        assert.strictEqual(puzzle.toString(),expected);
    ...
    it("covers toString output valid for kd-6-31-6.starb", async function () {
        const server = new WebServer(0);
        await server.start(); 
        const fileContents = await fetch(`http://localhost:${server.port}/get/kd-6-31-6.starb`);
        const puzzle: Puzzle = parsePuzzle(await (fileContents.text()));
        const expected =  "10x10\n" +
        "| 1,1 3,1 2,1 \n" + 
